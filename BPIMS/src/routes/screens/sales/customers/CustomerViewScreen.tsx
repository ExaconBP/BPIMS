...
const CustomerViewScreen = React.memo(({ route }: Props) => {
    ...
    return (
        <View className="flex flex-1">
            ...
            <TitleHeaderComponent showTrash={customer && customer.id != 0} onTrashPress={() => customer && removeCustomer(customer?.id)} isParent={false} userName={user.name} title={customer && customer.id != 0 ? customer.name : "New Customer"} onPress={() => navigation.goBack()}></TitleHeaderComponent>
