        ...
        if (editingField && returnStock) {
            if (item.sellByUnit) {
                ...
                if (Number(newValue) <= item.quantity) {
                    ...
                    setMessage(`Quantity exceeds available warehouse stock. Available stock: ${Math.round(item.quantity)}`);
                    return;
                }
            } else {
                ...
                if (Number(formatted) <= item.quantity) {
                    ...
                    setMessage(`Quantity exceeds available warehouse stock. Available stock: ${Number(item.quantity).toFixed(2)}`);
                    return;
                }
            }
        }
